...
@Autonomous(name ="Auto Controls Test n Align", group = "Bottom")
//@Disabled
...
public class AutoControlsMTZ_test_w_align extends LinearOpMode {
        ...
        if (pathToRun == "Backdrop" || pathToRun == "Audience" || pathToRun == "Backdrop Sample" || pathToRun == "Audience Sample"){
            ...
            if (pathToRun == "Backdrop Sample" || pathToRun == "Audience Sample"){ //Need to Sample
                ...
                if (randomNumberResult==1) {
                    ...
                    Drive(21, defaultDriveSpeed, defaultPauseTime); //Drive forward to center spike
                    Drive(-21, defaultDriveSpeed, defaultPauseTime); //Drop pixel and backup close to original position
                    Turn(30,defaultDriveSpeed,defaultPauseTime);
                }
                if (randomNumberResult==2) {
                    Drive(19, defaultDriveSpeed, defaultPauseTime); //Drive forward to center spike
                    Drive(-21, defaultDriveSpeed, defaultPauseTime); //Drop pixel and backup close to original position
                }
                ...
                Drive(36, defaultDriveSpeed, defaultPauseTime); //Drive forward to center spike
            ...
            //Strafe(-20*allianceReverser, defaultDriveSpeed, defaultPauseTime); //slide in front of backdrop. Strafe isn't working so we just took this one out.
            Turn(-30,defaultDriveSpeed,defaultPauseTime); //To look towards rest of field
            ...
            int aprilTagSpacing = 6; //set a distance between AprilTags on the backdrop
            ...
            Strafe(-1*backdropAprilTagTargetOffset*allianceReverser,defaultDriveSpeed,defaultPauseTime); //Slide over the offset amount

            RaiseArm(10,defaultPauseTime); //Raise arm
            Drive(15, defaultDriveSpeed/4, defaultPauseTime);
            //Deliver Pixel
            ...
            //Need aligning code
            if(allianceReverser==1) {
                DESIRED_TAG_ID = randomNumberResult + 3;
            } else {
                DESIRED_TAG_ID = randomNumberResult;
            }

            /*
            int alignTimer=10;

            while (opModeIsActive() && alignTimer>0)
            {
                targetFound = false;
                desiredTag  = null;
